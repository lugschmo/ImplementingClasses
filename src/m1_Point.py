...
class Point(object):
    """ Objects are individual points in a plane with x and y coordinates. """
    ...
    def move_to(self, x, y):
        self.x_distance = abs(x - self.x)
        self.y_distance = abs(y - self.y)
        self.distance_traveled = self.distance_traveled + math.sqrt(self.x_distance ** 2 + self.y_distance ** 2)
        self.x = x
        self.y = y
    ...
    def closer_to(self, p2, p3):
        p2_to_p1 = math.sqrt((p2.x - self.x) ** 2 + (p2.y - self.x) ** 2)
        p3_to_p1 = math.sqrt((p3.x - self.x) ** 2 + (p3.y - self.x) ** 2)
        if p2_to_p1 == p3_to_p1:
            return p2
        elif p2_to_p1 < p3_to_p1:
            return p2
        else:
            return p3
